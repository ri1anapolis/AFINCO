...
class CategoriaDespesaAutocomplete(LoginRequiredMixin, HasRoleMixin, autocomplete.Select2QuerySetView):
...
class HistoricoDespesasAutocomplete(LoginRequiredMixin, HasRoleMixin, autocomplete.Select2QuerySetView):
    allowed_roles = ['oficial', 'contador']
    ...
    def get_queryset(self):
        ...
        if self.q:
            qs = qs.filter(
                identificacao__icontains=self.q
            )
...
class CategoriaDespesaCreateView(LoginRequiredMixin, HasRoleMixin, SuccessMessageMixin, CreateView):
    allowed_roles = ['oficial', 'contador']
    model = models.CategoriaDespesa
...
class CategoriaDespesaUpdateView(LoginRequiredMixin, HasRoleMixin, SuccessMessageMixin, UpdateView):
    allowed_roles = ['oficial', 'contador']
    model = models.CategoriaDespesa
    form_class = forms.CategoriaDespesaForm
...
class CategoriaDespesaDeleteView(LoginRequiredMixin, HasRoleMixin, DeleteView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            categoria_link = '<a href="' + reverse('categoria_despesa_detail', args=[str(self.object.id)])
            categoria_link += f'" target="_blank">{self.object}</a>'
            ...
            return redirect(
                reverse('categoria_despesa_list')
            )
...
class DespesaCreateView(LoginRequiredMixin, HasRoleMixin, SuccessMessageMixin, CreateView):
    allowed_roles = ['oficial', 'contador']
    model = models.Despesa
...
class DespesaUpdateView(LoginRequiredMixin, HasRoleMixin, SuccessMessageMixin, UpdateView):
    allowed_roles = ['oficial', 'contador']
    model = models.Despesa
    form_class = forms.DespesaForm
